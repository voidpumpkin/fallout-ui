...
pub fn upload_file(input_ref: &NodeRef, upload_cb: Callback<Result<String, FileReadError>>) -> JsResult<callbacks::FileReader> {
        ...
        Some(input_elem) => match input_elem.files() {
            Some(file_list) => {
                ...
                match file_list.deref().first() {
                    Some(file) => Ok(callbacks::read_as_text(file.deref(), move |resp| upload_cb.emit(resp))),
