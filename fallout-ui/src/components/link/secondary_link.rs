...
#[function_component]
pub fn SecondaryLink<T: Routable + 'static>(props: &Props<T>) -> Html {
    let Props {
        class,
        children,
        to,
    ...
    let class = use_secondary_link_logic(class, false, None);

    let data_qa = to_data_qa(format!("{data_qa}-secondary-link"));
